...
export const EstimatesGrid = forwardRef<EstimatesGridHandle, Props>(function EstimatesGrid(
  { isbn, grid, expanded, onToggle, filter, canEdit, dirtyCells, savedCells, onEdit, highlightKey },
  ref,
) {
                ...
                {GRID_COLS.map((col, ci) => {
                  ...
                  return (
                    <Cell
                      ...
                      onToggle={onToggle}
                      dirty={col.field ? dirtyCells.has(cellId(isbn, row, col.field) ?? "") : false}
                      saved={col.field ? savedCells.has(cellId(isbn, row, col.field) ?? "") : false}
                      onMouseDown={() => {
...
function Cell({ row, col, hasComp, active, editable, expanded, dirty, saved, onToggle, onMouseDown, onDoubleClick, children }: CellProps) {
  ...
  if (col.kind === "name") content = <NameCell row={row} expanded={expanded} onToggle={onToggle} />;
  ...
  return (
    <div role="gridcell" className={base} onMouseDown={onMouseDown} onDoubleClick={onDoubleClick}>
      {content}
      {children}
    </div>
  );
...
function NameCell({ row, expanded, onToggle }: { row: GridRow; expanded: boolean; onToggle: (key: string) => void }) {
  if (row.kind === "total") {
    return (
